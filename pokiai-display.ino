...
File file;
char filename[32] = "/recording.wav";
...
void uploadFile(const char* uploadFilename){
  Serial.println("Uploading");
  file = SD.open(uploadFilename, FILE_READ);
  if(!file){
  ...
  HTTPClient client;
  // client.begin("http://10.0.0.85:8888/uploadAudio");
  client.begin("http://10.19.210.123:8888/uploadAudio"); 
...
void i2s_adc(void *arg, const char* filename)
{
...
void loop() {
  Serial.println(WiFi.macAddress());
	delay(1000);
    for (int i = 0; i < 5; i++) {
        char filename[32]; // Ensure the buffer is large enough
        sprintf(filename, "/recording_%d.wav", i); // Generate the filename
        ...
        // Start recording
        i2s_adc(NULL, filename); // Pass the filename to i2s_adc
        
        // Ensure WiFi is connected (might need to check and/or reconnect here)
        if (WiFi.status() == WL_CONNECTED) {
            // Upload the file
            uploadFile(filename); // Adjust this function as needed
        } else {
    ...
    Serial.println("Done with all recordings, uploads, and deletions.");
    while (1) {
        delay(1000); // Prevent a busy loop, or implement a more meaningful condition
    }
}
